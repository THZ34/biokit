...
def cooccurrence_lineplot(adata, groupby, groups1=None, groups2=None, color_dict=None, lineparams=None,
                          legendparams=None):
    if groups1 is None:
    ...
    fig, axes = create_fig(12, 4, len(groups1), 1)
    for group1, ax in zip(groups1, axes.flatten()):
        for group2 in groups2:
            temp_cooccurrence_df = co_occurrence_df[(co_occurrence_df['group1'] == group1) & (
                    co_occurrence_df['group2'] == group2)]
            ax.plot(temp_cooccurrence_df.columns[2:], temp_cooccurrence_df.values[0][2:], color=color_dict[group2],
                    label=group2, **lineparams)
            ax.legend(bbox_to_anchor=(1, 1), loc='upper left', **legendparams)
            plt.tight_layout()
