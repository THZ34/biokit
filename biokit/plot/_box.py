...
def testbox(data, y, x=None, x_order=None, groupby=None, groups=None, kind='box', testfunc=ttest_ind, cutoff=None,
    ...
    if kind == 'box':
        sns.boxplot(width=width, showfliers=showfliers, **plot_kws, )
    elif kind == 'violin':
        sns.violinplot(cut=0, linewidth=0, **plot_kws, )
        sns.boxplot(width=0.8, showfliers=False, color=None, gap=0.7, boxprops={"facecolor": "none"},
                    capprops={"linewidth": 0}, **plot_kws, )
    elif kind == 'bar':
        sns.barplot(capsize=0.2, **kwargs, **plot_kws)
    elif kind == 'strip':
    ...
    if kind == 'bar':
        ...
        ymin, ymax = map(float, ax.dataLim.intervaly)
